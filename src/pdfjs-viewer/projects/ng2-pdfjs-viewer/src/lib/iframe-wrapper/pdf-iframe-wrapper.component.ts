...
export class PdfIframeWrapperComponent implements OnInit
{
    ...
    private readonly _pendingAnnotationNotSet = 'Expected the pending annotation to be set.';
    ...
    @Output() startNewAnnotation = new EventEmitter<void>();
    ...
    public disableButton(buttonType: toolbarButtonType, disabled: boolean)
    {
        ...
        buttonids.forEach(x => {
            ...
            // Extra check if required to stop ESLint from complaining.
            if (!this.pdfBehaviour.iframeDocument) {
                throw new Error(this._iframeDocumentNotLoadedError);
            }
            ...
            if (!element) {
                throw new Error(`Expected element with id '${x}' to exist.`);
            }
    ...
    public enableAnnotationColor(annotation: pdfAnnotation, color: string)
    {
        ...
        this.pdfAnnotationWriter.colorAnnotation(annotation, color);
    ...
    private iframeLoaded()
    {
        ...
        // Role does not exist on a button by default, apparently.
    ...
    private setPendingAnnotationTextSelection(selection: Selection)
    {
        ...
        if (!this.pendingAnnotation) {
            throw new Error(this._pendingAnnotationNotSet);
        }
        ...
        this.pendingAnnotationTextSelected.emit();
    ...
    private onBoundingBoxCreated(bounds: boundingBox, page: number)
    {
        ...
        this.pendingAnnotationBoundingBoxCreated.emit();
    }
